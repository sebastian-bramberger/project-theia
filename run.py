...
def build_sin(run_args, sub_args):
    ...
    command += [output_file, "apptainer_recipe"]
    print(f"running: {' '.join(command)}")
...
def train(run_args, sub_args):
    path = Path(__file__).parent.joinpath("project_theia/train.py").absolute()
...
def resume(run_args, sub_args):
    path = Path(__file__).parent.joinpath("project_theia/resume.py").absolute()
    command = env_prefix(run_args.env) + ["python3", "-u", str(path)]
    command += sub_args
    print(f"running: {' '.join(command)}")
    subprocess.run(command)
...
def evaluate(run_args, sub_args):
    path = Path(__file__).parent.joinpath("project_theia/evaluate.py").absolute()
    command = env_prefix(run_args.env) + ["python3", "-u", str(path)]
    command += sub_args
    print(f"running: {' '.join(command)}")
    subprocess.run(command)
